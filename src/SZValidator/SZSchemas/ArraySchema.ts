...
export class ArrayItemSchemaMissing extends Error {
...
export class ArraySchema extends AnySchema {
  ...
  _sparseMode:boolean = false;
  _singleMode:boolean = false;
  _itemSchemas:Array<AnySchema> = [];
  _comparatorFunction:Function;
  _comparatorKey:string;
  _ordered:Array<AnySchema> = [];

  public internalJSONSchema:SZArray<any>;

  constructor() {

    super(SZSchemaTypes.Array);

    this.internalJSONSchema = new SZArray();

  }

  sparse(sparseModeOn?:boolean) {
  ...
  items(...itemSchemas:Array<AnySchema>) {
    ...
    if (this._itemSchemas.length > 1) {
      ...
      this.internalJSONSchema.setItems.apply(this.internalJSONSchema, map(this._itemSchemas, "internalJSONSchema"));
  ...
  ordered(...orderedSchemas:Array<AnySchema>) {
    ...
    if (orderedSchemas.length === 1) {
      ...
      this.internalJSONSchema.setOrdered(map(orderedSchemas, "internalJSONSchema"));
  ...
  unique(comparator?:Function|string) {
  ...
  validate(value:Array<any>|any) {
    ...
    if (!valIsNullOrUndefined && !valIsArray && !this._singleMode) {
      ...
      throw new SchemaValidationFailedError(`Value passed wasn't an array and single mode isn't enabled.`);

    }
    else if (valIsNullOrUndefined && this.internalJSONSchema._requiredFlag === true && !this._sparseMode) {

      throw new SchemaValidationFailedError(`Value is required.`);

    }
    else if (!valIsArray && this._singleMode) {

      if (this._ordered.length > 0 && this._ordered[0] instanceof AnySchema) {

        return AnySchema.postValidate(this,[this._ordered[0].validate(valStack.value)], value);

      }

      return AnySchema.postValidate(this,[this.itemValidate(valStack.value)], value);

    }
    else if (valIsArray && value.length === 0 && !this._sparseMode) {

      throw new SchemaValidationFailedError(`Value passed is an empty array and sparse mode isn't enabled.`);
      ...
      valStack.value.forEach((item, index) => {
        ...
        try {
          ...
          if (orderedMode && self._ordered[index] && self._ordered[index] instanceof AnySchema) {
          ...
          if (ex.name === "ArrayItemSchemaMissing") throw ex;
      ...
      return AnySchema.postValidate(this,validatedArray, value);

    }

    throw new SchemaValidationFailedError(`Value passed is not valid for ArraySchema validation.`);
